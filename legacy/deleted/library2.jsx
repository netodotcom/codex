...
function LibraryX() {
  ...
  const goChapter = (b, ch) => {
    ...
    if (window.codexGoto) window.codexGoto(b.id, ch, 1);
    else if (window.codexJumpToRef) window.codexJumpToRef(`${b.name} ${ch}`);
  };
    ...
    return (
      <div key={b.id} className={`cxl-book ${open ? "is-open" : ""}`}>
        <button
          ...
          aria-expanded={open}
        >
          <span className={`cxl-light is-${light}`} aria-hidden
            title={light === "own" ? `In ${(primary || "").toUpperCase()}` : light === "other" ? "Served from another corpus" : "No known source"} />
          <span className="cxl-name">{b.name}</span>
          <span className="cxl-meta">{active ? `${now.chapter} / ${b.chapters}` : b.chapters}</span>
        </button>
        {open ? (
          <div className="cxl-chs" role="group" aria-label={`${b.name} chapters`}>
            {light === "none" ? (
              <div className="cxl-nosource">○ no corpus in the registry carries {b.name} yet — install one via the marketplace</div>
            ) : null}
    ...
    <div className="cxl">
      <div className="cxl-search">
        <input
          ...
          spellCheck={false}
        />
        {q ? <button className="cxl-clear" onClick={() => setQ("")} aria-label="Clear">×</button> : null}
      </div>

      {refJump ? (
        <button className="cxl-jump" onClick={() => { goChapter(refJump.b, refJump.ch); if (refJump.v && window.codexJumpToRef) window.codexJumpToRef(`${refJump.b.name} ${refJump.ch}:${refJump.v}`); setQ(""); }}>
          ✦ OPEN {refJump.b.name.toUpperCase()} {refJump.ch}{refJump.v ? ":" + refJump.v : ""}
        </button>
      ) : null}
      {hits.length ? (
        <div className="cxl-hits" role="list">
          {hits.map((h, i) => {
            ...
            if (!b) return null;
            return (
              <button key={i} className="cxl-hit" role="listitem"
                onClick={() => { if (window.codexJumpToRef) window.codexJumpToRef(`${b.name} ${p[1]}:${p[2]}`); setQ(""); }}>
                <b>{b.name} {p[1]}:{p[2]}</b>
      ...
      <footer className="cxl-foot" aria-hidden>
