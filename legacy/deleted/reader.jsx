...
function CxrText({ text, gold }) {
  ...
  if (segs.length === 1 && !segs[0].kind) return <span className="cxr-text">{segs[0].t}</span>;
  return (
    ...
    <span className="cxr-spawn" ref={ref}>
      ...
      {open ? (
        <span className="cxr-spawn-pop" role="menu">
          <button role="menuitem" onClick={() => { setOpen(false); if (window.codexNewReader) window.codexNewReader(); }}>
...
function CodexReaderX({ surface, independent, initialNow, onNowChange } = {}) {
  ...
  useEffect(() => {
    ...
    if (P && P.subscribe) {
      unsub = P.subscribe((ev) => {
        ...
        else if (ev.type === "done") setPanelS({ data: ev.data, late: true, loading: false });
  ...
  const renderGlosses = (list) => (list || []).map(g => (
  ...
  return (
    <div className={`cxr ${surface ? "is-" + surface : ""} ${independent ? "is-pinned" : ""}`} style={{ "--cxr-fs": fs + "px" }}>
