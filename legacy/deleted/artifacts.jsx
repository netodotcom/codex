  ...
  return (
    ...
    >
      ...
      {open ? ReactDOM.createPortal(
        <span className="cx-art-ref-pop" role="tooltip"
          ...
          <span className="cx-art-ref-pop-b">
            {preview === null ? "loading…"
              ...
              : preview.map(v => <span key={v.n}><sup>{v.n}</sup>{v.text} </span>)}
          </span>
        </span>,
        document.body
      ) : null}
...
function artRenderInline(text) {
  ...
  for (const b of blocks) {
    if (b.kind) {
      out.push(<span key={nextKey()} className={b.kind === "red" ? "cx-red" : "cx-divine"}>{b.text}</span>);
      continue;
    }
...
function artRunAction(action) {
  ...
  try {
    if (kind === "goto") {
      if (action.ref) { if (window.codexJumpToRef) window.codexJumpToRef(String(action.ref)); }
      else if (action.book && window.CODEX_KERNEL && window.CODEX_KERNEL.parseRef) {
...
function ArtButtons({ code }) {
  ...
  return (
    <div className="cx-art-buttons" role="group" aria-label="Suggested actions">
      {good.map((b, i) => (
        <button key={i} type="button" className="cx-art-btn"
  ...
  if (type === "line") {
    ...
    svg = (
      <svg viewBox={`0 0 ${W} ${H}`} width={W} role="img" aria-label={`Line chart${title ? ": " + title : ""}`}>
        ...
        <polyline fill="none" stroke={ART_ACCENT} strokeWidth="1.6"
    ...
    svg = (
      <svg viewBox={`0 0 ${W} ${H}`} width={W} role="img" aria-label={`Radial chart${title ? ": " + title : ""}`}>
        {segs.map((s, i) => <path key={i} d={s.d} fill={s.hue} opacity="0.85" />)}
        {segs.map((s, i) => (
          <g key={"l" + i}>
            <rect x={176} y={14 + i * 18} width="8" height="8" fill={s.hue} />
            <text x={190} y={22 + i * 18} fontSize="9" fill={ART_FG}>
              {String(s.item.label).slice(0, 18)} · {artFmt(s.item.value)}
            </text>
          </g>
        ))}
      </svg>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              <rect x={labelW} y={y + 5} width={w} height={12} fill={ART_HUES[i % ART_HUES.length]} opacity="0.85" rx="1.5" />
...
function ArtFlow({ code }) {
  ...
  return (
    <figure className="cx-art-flow">
      <svg viewBox={`0 0 ${W} ${H}`} width={Math.min(W, 560)} role="img"
        ...
        {edges.map((e, i) => {
          ...
          return (
            <g key={"e" + i}>
              ...
              {e.label ? (
                <text x={back ? (a.x + b.x + NW) / 2 : mx} y={back ? (a.y + NH + b.y) / 2 + 3 : (y0 + y1) / 2 - 4}
                  fontSize="7.5" fill={ART_DIM} textAnchor="middle">{e.label.slice(0, 22)}</text>
              ) : null}
            </g>
          );
        })}
        {nodes.map((n) => {
          ...
          return (
            <g key={n.id}
              ...
              onKeyDown={clickable ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); if (window.codexJumpToRef) window.codexJumpToRef(String(n.ref)); } } : undefined}>
              <rect x={p.x} y={p.y} width={NW} height={NH} rx="5"
                fill="rgba(126,224,255,0.07)" stroke={clickable ? ART_ACCENT : "rgba(126,224,255,0.4)"} strokeWidth="1" />
...
function ArtVerseCard({ refStr }) {
  ...
  const jump = () => {
    if (parsed && window.codexGoto) window.codexGoto(parsed.bookId, parsed.chapter, parsed.v1 || 1);
    else if (window.codexJumpToRef) window.codexJumpToRef(refStr);
  };
  ...
  return (
    <button type="button" className="cx-art-vcard" onClick={jump} title={`Open the reader at ${refStr}`}>
      ...
      {state.err ? <span className="cx-art-vcard-txt is-dim">(text unavailable here — tap to open the reader)</span>
        : state.lines === null ? <span className="cx-art-vcard-txt is-dim">loading…</span>
        : <span className="cx-art-vcard-txt">{state.lines.map(l => <span key={l.n}><sup style={{ opacity: 0.6 }}>{l.n}</sup> {l.text} </span>)}</span>}
    </button>
  );
}
...
function ArtVerseGrid({ code }) {
  ...
  return (
    <div className="cx-art-vgrid">
      {good.map((r, i) => <ArtVerseCard key={r + i} refStr={r.trim()} />)}
    </div>
  );
}
...
function artRenderFence(b, key) {
  ...
  return <pre key={key} className="cx-art-code">{b.code}</pre>;
}
  ...
  return (
    <div className="cx-art">
      {blocks.map((b, i) => {
        ...
        if (b.type === "h") return <div key={i} role="heading" aria-level={Math.min(b.level + 2, 6)} className={`cx-art-h is-${Math.min(b.level, 4)}`}>{artRenderInline(b.text)}</div>;
        if (b.type === "hr") return <hr key={i} />;
        if (b.type === "quote") return <blockquote key={i}>{artRenderInline(b.text)}</blockquote>;
        if (b.type === "list") {
          const Tag = b.ordered ? "ol" : "ul";
          return <Tag key={i}>{b.items.map((it, j) => <li key={j}>{artRenderInline(it)}</li>)}</Tag>;
        }
        if (b.type === "table") {
          return (
            <table key={i} className="cx-art-table">
              <thead><tr>{b.head.map((h, j) => <th key={j}>{artRenderInline(h)}</th>)}</tr></thead>
              <tbody>{b.rows.map((r, j) => <tr key={j}>{r.map((c, k) => <td key={k}>{artRenderInline(c)}</td>)}</tr>)}</tbody>
            </table>
          );
        }
